...
export function buildGroupCtx(xcasper, msg, args, prefix) {
    ...
    const sender = rawSender.split(':')[0] + (rawSender.includes('@') ? (rawSender.endsWith('@lid') ? '@lid' : '@s.whatsapp.net') : '@s.whatsapp.net');
...
export async function fetchGroupCtx(xcasper, msg, ctx) {
    ...
    const rawParticipant = msg.key?.participant || '';
    const rawSender = isGroup ? rawParticipant : from;
    const senderJid = rawSender.split(':')[0] + (rawSender.includes('@') ? (rawSender.endsWith('@lid') ? '@lid' : '@s.whatsapp.net') : '@s.whatsapp.net');
    ...
    if (isGroup) {
        ...
        groupSuperAdmins = participants.filter(p => p.admin === 'superadmin').map(p => p.id);
        const botJid = (xcasper.user?.id?.split(':')[0] || '') + '@s.whatsapp.net';
        isBotAdmin = groupAdmins.includes(botJid);
        const senderNum = senderJid.split('@')[0];
        isAdmin = groupAdmins.some(a => a.split('@')[0] === senderNum);
        isSuperAdmin = groupSuperAdmins.some(a => a.split('@')[0] === senderNum);
    }
    ...
    return { groupMetadata, groupAdmins, groupSuperAdmins, participants, isBotAdmin, isAdmin, isSuperAdmin, isSuperUser, superUser, botName, senderJid };
}
